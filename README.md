...
This project includes basic machine learning capabilities for health data analysis:

- **Abnormal detection model** — classifies readings as normal vs abnormal using Random Forest
- **Future prediction** — predicts next biomarker values using Linear Regression
- **Health risk scoring** — computes a cumulative risk score based on abnormal readings
- **Real-time inference** — predictions are generated dynamically in the dashboard API

## 🏗️ Architecture
- **Data Layer**
  - Multiple input formats (CSV, XPT, PDF, JSON)
  - Unified into `health_data.csv`

- **Processing Layer**
  - Python scripts for normalization & cleaning
...
- Accepts **multi-format medical data**
- Performs **automatic normalization**
- Provides **AI-based predictions**
- Runs **fully locally (privacy-first)**
- Requires **no backend framework or database**
