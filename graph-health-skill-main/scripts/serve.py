...
def compute_ai(csv_path):
    ...
    for test in df["test_name"].unique():
        ...
        try:
            ...
            next_time = pd.DataFrame([[sub["time"].max() + 1]], columns=["time"])
            future_val = model.predict(next_time)[0]

            predictions[test] = round(float(future_val), 2)
        except:
            continue
...
def make_handler(csv_path: Path):
    class Handler(http.server.SimpleHTTPRequestHandler):
        ...
        def _send_data(self):
            ...
            # ✅ FIX: define context BEFORE using it
            context = None
